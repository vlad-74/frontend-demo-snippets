...
) ELSE IF %cd%==D:\_DEVELOPER\Sites\_GIT\frontend-demo-snippets\demo\##mongoose_session_cookie (
   start %location%\dir\mongodb\mongod.bat
   timeout 8 >nul
   start %location%\dir\mongodb\mongo.bat
) ELSE IF %cd%==D:\_DEVELOPER\Sites\_GIT\frontend-demo-snippets\demo\##mongoose_session_cookie\dir\mongodb (
