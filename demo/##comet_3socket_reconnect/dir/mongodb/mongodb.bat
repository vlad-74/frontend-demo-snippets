...
) ELSE IF %cd%==D:\_DEVELOPER\Sites\_GIT\frontend-demo-snippets\demo\##comet_3socket_reconnect (
   start %location%\dir\mongodb\mongod.bat
   timeout 8 >nul
   start %location%\dir\mongodb\mongo.bat
) ELSE IF %cd%==D:\_DEVELOPER\Sites\_GIT\frontend-demo-snippets\demo\##comet_3socket_reconnect\dir\mongodb (
