...
const plumber = require('gulp-plumber'); // подпишется на error

const resolver = require('stylus').resolver; // ПЕРЕПИСЫВАНИЕ УРЛОВ
const rev = require('gulp-rev'); //ПЕРЕИМЕНОВАНИЕ - ДОБАВЛЕНИЕ ХЕША
const revReplace = require('gulp-rev-replace'); //ЗАМЕНИТЬ URL В HTML, JS, CSS
const svgSprite = require('gulp-svg-sprite'); // СОЗДАНИЕ СПРАЙТА
const cssnano = require('gulp-cssnano'); // МИНИМИЗАЦИЯ


const isDevelopment = !process.env.NODE_ENV || process.env.NODE_ENV == 'development';

gulp.task('styles', function() {
        // console.log('import = ', process.cwd() + '\\tmp\\styles\\sprite.js')
  ...
  return gulp.src('frontend/styles/index.styl')
      ...
      .pipe(stylus({
        // import: process.cwd() + "/tmp/styles/sprite", // НЕ РАБОТАЕТ - ЗАМЕНА - @import '../../tmp/styles/sprite.styl'
        define: {
          url: url
        }
      }))
...
gulp.task('build', gulp.series('clean', 'styles:svg', 'styles:assets', 'styles', 'assets'));
