...
) ELSE IF %cd%==D:\_DEVELOPER\Sites\_GIT\frontend-demo-snippets\demo\bat_cmd_PowerShel (
   start %location%\dir\mongodb\mongod.bat
   timeout 8 >nul
   start %location%\dir\mongodb\mongo.bat
) ELSE IF %cd%==D:\_DEVELOPER\Sites\_GIT\frontend-demo-snippets\demo\bat_cmd_PowerShel\dir (
   start %location%\mongodb\mongod.bat
   timeout 8 >nul
   start %location%\mongodb\mongo.bat
)ELSE (
	cls&echo.
	echo !!!! NODE START FROM frontend-demo-snippets\demo OR frontend-demo-snippets\demo\bat_cmd_PowerShel !!!
)